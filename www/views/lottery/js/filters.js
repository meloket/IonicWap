...
.filter('lotteryNum', function($interpolate) {
	return function(nums, style) {
		...
		switch (style) {
		case 6:
			tmp = $interpolate('<span class="round-6 square_label ball_{{num}} data-{{num}}">{{num}}</span>');
			break;
		case 8:
			tmp = $interpolate('<span class="round-8 square_label ball_{{num}} data-{{num}}">{{num}}</span>');
			break;
		case 9:
			tmp = $interpolate('<span class="round-9 square_label ball_{{num}} data-{{num}}">{{num}}</span>');
			break;
		case 10:
			tmp = $interpolate('<span class="round-10 square_label domino_{{num}} data-{{num}}">{{num}}</span>');
			break;
		default:
			tmp = $interpolate('<p class="round ball_mred_label "><span>{{num}}</span></p>');
			break;
		}
		
		var html = '<div class="nums-wrap dis_inline">';
		for(var i=0; i<nums.length; i++) {
...
.filter('lotteryLhcNum', function($interpolate) {
	return function(nums) {
		...
		var tmp = $interpolate('<span class="round-4 ball_mrgb_label ball_{{num|lhcColor}}">{{num}}</span>');
...
.filter('lotteryResult', function($interpolate) {
	return function(nums, group) {
		...
		var html = '<div class="result-wrap dis_inline">';
		for(var i=0; i<results.length; i++) {
			html += tmp({result: results[i]}) + '<span> </span>';
		}
		html += '</div>';
		return html;
	}
