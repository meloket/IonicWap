...
.controller('LotteryCtrl', function($rootScope, $scope, $timeout, $log, $filter, $stateParams, $ionicPopover, $ionicHistory, Lottery, UCenter, $state, Tools, My, $ionicScrollDelegate) {
	$log.debug("LotteryCtrl...");

	$rootScope.myValue = false;
	$scope.gameppt = true;
	$scope.playerppt = false;
	...
	$scope.betlikethis = function() {
		console.log($scope.BetStatusData.betid);
		Tools.ajax({
			url: '/data/postcodeagent',
			backdorp: true,
			dataType: "json",
			params: {betid: $scope.BetStatusData.betid, userid: My.getUserId(), money: $scope.BetStatusDataChangeMoney},
			success: function(g) {
				console.log(g);
				My.refreshMoney(false);
				var dlgElem = angular.element("#modal_dlg_gentou");
				if (dlgElem) {
				   dlgElem.modal("hide");
				}
			}
	...
	$scope.openDlg = function (val) {
		$scope.BetStatusData = val;
		$scope.BetStatusDataChangeMoney = parseFloat(val.money);
		// console.log(val);
		var dlgElem = angular.element("#modal_dlg_gentou");
		if (dlgElem) {
		   dlgElem.modal("show");
	...
	$scope.BetStatusData = [];
	$scope.BetStatusDataChangeMoney = 0.0;
	...
	$scope.reset = function() {
		...
		$('.bet-view').find('.highlight').removeClass('highlight');
	};
...
.controller('NotcountCtrl', function($scope, $log, Tools, Lottery, $state, $ionicHistory) {
...
.controller('NotcountDetailCtrl', function($scope, $log, $stateParams, Tools, Lottery, $state, $ionicHistory) {
	$scope.gameId = $stateParams.gameId;
...
.controller('HuodongCtrl', function($rootScope, $scope, $log, $stateParams, Tools, Lottery, $state, $ionicHistory) {
	$scope.queryData = {};
	$rootScope.myValue = false;
...
.controller('TaskCtrl', function($rootScope, $scope, $log, $stateParams, Tools, Lottery, $state, $ionicHistory) {
	$scope.queryData = {};
	$scope.btval = 1;
	$scope.buttonStatus1 = true;
	$scope.buttonStatus2 = false;
	$scope.buttonStatus3 = false;
	$rootScope.myValue = false;

	var mybt1 = angular.element( document.querySelector( '#bt1' ) );
	var mybt2 = angular.element( document.querySelector( '#bt2' ) );
	var mybt3 = angular.element( document.querySelector( '#bt3' ) );
	
	$ionicHistory.nextViewOptions({
		disableAnimate: true
	});

	$scope.BtClickHandler = function(){
		// alert("Inside controller: value of myval is " + $scope.btval);
		if($scope.btval==1){
			mybt2.removeClass('taskTitleActive');
			mybt3.removeClass('taskTitleActive');
			mybt1.addClass('taskTitleActive');
			$scope.buttonStatus2 = false;
			$scope.buttonStatus3 = false;
			$scope.buttonStatus1 = true;
		} else if($scope.btval==2){
			mybt1.removeClass('taskTitleActive');
			mybt3.removeClass('taskTitleActive');
			mybt2.addClass('taskTitleActive');
			$scope.buttonStatus1 = false;
			$scope.buttonStatus3 = false;
			$scope.buttonStatus2 = true;
		} else if($scope.btval==3){
			mybt1.removeClass('taskTitleActive');
			mybt2.removeClass('taskTitleActive');
			mybt3.addClass('taskTitleActive');
			$scope.buttonStatus1 = false;
			$scope.buttonStatus2 = false;
			$scope.buttonStatus3 = true;
		}
	}		
})

.controller('SettledCtrl', function($scope, $log, Tools, Lottery, $state, UCenter, $filter, $timeout, $ionicHistory) {
...
.controller('HistoryCtrl', function($rootScope, $scope, $log, $stateParams, $state, $ionicHistory, Tools, Lottery) {
	$scope.queryData = {};
	...
	var getHistoryUrl = function(gameId, date) {
		...
		return base_url + Tools.staticPath() + 'data/HistoryLottery.js?dateStr=' + date + '&gameId=' + gameId + '&_' + Math.random();
	};

	var getDateParam = function(date) {
		if(!$scope.showDate) {
			date = '';
		}
		return date;
	...
	$scope.onQuery = function() {
		...
		// });
		Lottery.getHistoryJS(function(data) {
		...
		}, getDateParam($scope.queryData.date), $scope.queryData.gameId);
	};
...
.controller('ChangLongCtrl', function($scope, $log, $stateParams, Tools, Lottery, $state, $ionicHistory) {
	$scope.queryData = {};
	$scope.queryData.gameId = parseInt($stateParams.gameId || Lottery.getFirstGameId());
...
.controller('LuZhuCtrl', function($scope, $log, $stateParams, Tools, Lottery, $state, $ionicHistory) {
	...
	for(var g in gameList){
		var game = gameList[g];
		if(game.id==10 || game.id==710){//去除江苏快3 、 香港六合彩
			continue;
		}
	...
	$scope.queryData = {};
	$scope.queryData.gameId = parseInt($stateParams.gameId || Lottery.getFirstGameId());
	if($scope.queryData.gameId==10 || $scope.queryData.gameId==710){
		Tools.tip('无此类游戏路珠');
...
.controller('WeekRecordCtrl', function($rootScope, $scope, $state, $ionicHistory, UCenter, Tools, My) {
...
.controller('DayRecordCtrl', function($scope, $log, Tools, Lottery, $state, $ionicHistory, $stateParams) {
...
.controller('DayDetailCtrl', function($scope, $log, $state, $stateParams, $ionicHistory, Tools, Lottery) {
