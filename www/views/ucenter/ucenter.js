...
.controller('UCenterCtrl', function($rootScope, $scope, $timeout, $state, $ionicPopup, Tools, md5, My, UCenter, $location, $ionicHistory) {
	$scope.base_url = base_url;
	$scope.agenturl = base_url + "/user/regist?uid=" + My.getUserId();
	...
	$scope.updatePwd = function() {
		...
		Tools.ajax({
			...
			success: function(data) {
				if(data=='ok') {
					Tools.tip('修改密码成功');
					$('#modal_dlg_change_login_pass').modal('hide');
		...
		Tools.ajax({
			...
			success: function(data) {
				if(data == "ok") {
					...
					$scope.fundPwdFormData={};
					Tools.tip("保存取款密码成功！");
					$('#modal_dlg_change_money_pass').modal('hide');
					// $state.go("ucenter.index");
				}else{
					Tools.tip(data);
					return false;
...
.controller('NoticeCtroller', function($rootScope, $scope, $state, UCenter, $timeout, $ionicHistory, $ionicModal, Tools) {
	// 消息中心
...
//	        	var bl = /^[a-zA-Z ]{1,20}$/.test(realname) || /^[\u4e00-\u9fa5]{1,10}$/.test(realname);
	...
	this.checkFundPwd = function() {
		...
		if (!My.hasFundPwd()) {
			Tools.tip("请先设置取款密码！");
			$state.go("ucenter.myfpwd"); // 需要设置取款密码
			return false;
		} else {
			return true;
		}
