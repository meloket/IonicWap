...
.run(function($ionicPlatform, $ionicHistory, $rootScope, $log, PATH, ROUTE_ACCESS, Tools) {
...
.controller('LandCtrl', function($scope, $rootScope, $location, $ionicHistory, $log, Tools, PATH, My, md5) {
	$scope.My = My;
...
.controller('HomeCtrl', function($rootScope, $scope, $log, My, PATH, $location, $ionicHistory, $timeout, Tools, $ionicSlideBoxDelegate) {
...
.controller('AppCtrl', function($scope, $rootScope, $location, $ionicHistory, $log, $cookies, Tools, PATH, My, md5) {
...
// 	$log.debug('AppCtrl');
// 	console.info("ACD");
// 	// console.info(base_url + Tools.staticPath() + "data/configjs.js?_" + Math.random());

// 	// alert("JJJJJJJJJJ");

// 	// $http.get(base_url + Tools.staticPath() + "data/configjs.js?_" + Math.random())
//     //     .success(function (data) {
// 	// 		console.info("NNNNNNNNNN");
// 	// 		console.info(data);	
// 	// 	});
	
// 	// require([base_url + Tools.staticPath() + "data/configjs.js?_" + Math.random()], function(CONFIG_MAP) {
// 	// 	alert("OKOKKKO");
// 	// 	CONFIG_MAP.init();
// 	// 	alert(CONFIG_MAP);
...
// // [base_url + Tools.staticPath() + "data/configjs.js?_" + Math.random()]

	Tools.lazyLoad(["staticdata/configjs.js"], function() {

		// var $injector = angular.injector();
		// var CONFIG_MAP = $injector.get('CONFIG_MAP');
		// alert("EVery Called!");

		// console.info("BCD");
		// console.info(CONFIG_MAP);
		$rootScope.appConfig = angular.extend(CONFIG_MAP, appConfig);
		// console.info($rootScope.appConfig);
		// console.info("CCD");
		...
		// Tools.lazyLoad([base_url + Tools.staticPath() + "data/gamedatas.js"]);
		// Tools.lazyLoad(["staticdata/gamedatas.js"]);
	});
	...
	// Tools.lazyLoad([base_url + Tools.staticPath() + "data/configjs.js?_" + Math.random()], function() {
	// 	$rootScope.appConfig = angular.extend(CONFIG_MAP, appConfig);
	// 	My.init();
	...
	// 	My.getGamedatasJS(function(gdata) {
	// 		var goScript = document.createElement("script");
	// 		var goScriptText = document.createTextNode(gdata);
	// 		goScript.appendChild(goScriptText);
...
.controller('LoginCtrl', function($rootScope, $scope, $location, $ionicHistory, $log, Tools, PATH, $cookies, My, md5) {
	...
	$scope.login = function() {
		...
		Tools.ajax({
			...
			backdrop: true,
			params: {
				txt_login_user: $scope.loginData.username, 
				txt_login_password: $scope.loginData.password, 
				// pwdtext: $scope.loginData.password,
				// vcode: $scope.loginData.vcode
			},
			success: function(data) {
				console.info("logined tottot!");
				console.info(data);
				if(data.msg=="密码不正确" || data.msg=="用户名或密码不正确" || data.msg=="不允许空密码登录" || data.msg=="用户名包含非法字符,请重新登陆" || data.msg=="已经登录了!"){
					console.info("LOGINED FAILED!: " + data);
					$scope.msg = data.msg;
					return;
				}
				// $cookies.put('token', data.data.token);
				// console.info($cookies.get('token'));
				// console.info("Login_Function_END!");
				...
				My.getConfigJS(function(data) {
					...
					My.getParamJS(function(pdata) {
						var poScript = document.createElement("script");
						var poScriptText = document.createTextNode(pdata);
						poScript.appendChild(poScriptText);
						document.head.appendChild(poScript);
					});
	...
	$scope.loginwithqr = function(arg) {

		$scope.msg = '';
		// alert("loginedtowithqr: " + arg);
		Tools.ajax({
			method: 'POST',
			url: '/user/loginedtowithqr',
			backdrop: true,
			params: {
				txt_login_user: arg, 
				// txt_login_password: $scope.loginData.password, 
...
.controller('RegCtrl', function($scope, $rootScope, $location, $ionicHistory, $log, $timeout, Tools, My, PATH, $cookies, md5, $ionicScrollDelegate) {
...
.directive('zlNotice', function(Tools) {
	return {
		...
		link: function(scope, element, attrs) {
			...
			//[base_url + Tools.staticPath() + "data/messages.js?_" + Math.random()]

			Tools.lazyLoad(["staticdata/messages.js"], function() {
	...
	return {
		...
		$get: function(ENV, $rootScope, $http, $state, $injector, $location, $log, $ionicPopup, $ocLazyLoad, $ionicLoading, PATH, $cookies) {
			...
			var ajax = function(opts) {
				...
				var req = {method: method, url: base_url + apiPath() + opts.url, headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}};
				...
				// console.info("API_TOOL_AJAX: " + token);
				// console.info(req);
				console.log(req.url);
...
.config(function($stateProvider, $urlRouterProvider, ROUTE_ACCESS) {
	...
	.state('ucenter.intro', {
		...
		views: {
    		'ucenter-view': {
    			templateUrl: 'views/ucenter/intro.html',
    			controller: 'UCenterCtrl'
    		}
    	}
    })
	...
	.state('ucenter.agentshare', {
		url: '/agentshare',
		cache: false,
    	data : {access : ROUTE_ACCESS.CHECK_TEST},
    	views: {
    		'ucenter-view': {
    			templateUrl: 'views/ucenter/agentshare.html',
    			controller: 'UCenterCtrl'
    		}
    	}
    })

    .state('ucenter.agentrole', {
		url: '/agentrole',
		cache: false,
    	data : {access : ROUTE_ACCESS.CHECK_TEST},
    	views: {
    		'ucenter-view': {
    			templateUrl: 'views/ucenter/agentrole.html',
    			controller: 'UCenterCtrl'
    		}
    	}
    })
...
.config(function($stateProvider, $urlRouterProvider, ToolsProvider, ROUTE_ACCESS) {
	...
	.state('lottery', {
		...
		resolve : {
			...
			init: ['$q', 'Tools', function ($q, Tools) {
				...
				//base_url + Tools.staticPath() + "data/gamedatas.js"
		        Tools.lazyLoad(["staticdata/gamedatas.js"], function() {
					Backdrop.hide(500);
					deferred.resolve();
				});
	...
	.state('lottery.huodong', {
		url: '/huodong',
		cache: false,
		views: {
			'lottery-view': {
				templateUrl: 'views/lottery/huodong.html',
	...
	.state('lottery.task', {
		...
		views: {
			'lottery-view': {
				templateUrl: 'views/lottery/task.html',
				controller: 'TaskCtrl'
			}
		}
	})
...
.service('My', function($rootScope, $location, $interval, $log, $filter, $cookies, Tools, PATH) {
	...
	this.info = {};
	var bank = {};
	$rootScope.msgTimer = null;
	var startMsgTimer = function() {
		// console.log("NEXT STEP");
		// console.info(self.info.token);

		$rootScope.msgTimer = $interval(function() {
			// console.info("First_TokenTEST");
			// console.info(self.info.token);
			Tools.ajax({
				url: '/game/getUserMsg.do',
				success: function(data) {
					console.info("MSG_UPDATE_SUCCESS");
					console.info(data);
					if(data.balance) {
				...
				error: function() {
					...
					console.info("MSG_UPDATE_ERROR");
				}
			});
			console.info("TokenTEST: " + self.info.token);
			
			// if(!$cookies.get('token')){
			// 	$cookies.put('token', self.info.token);
			// 	alert("您已经退出登录，请重新登录");
			// 	console.log("OH!!!!!!!!!!!!!ssa: " + $cookies.get('token'));
	...
	this.init = function() {
		...
		// alert(token);
		// console.log("MY_COOKIE_TOKEN: " + token);
	...
	this.loginSuccess = function(data) {
		...
		$rootScope.myValue = true;
		console.log("tokenSVVSE: " + $cookies.get('token'));
		self.clearMsgTimer();
		startMsgTimer();
		checkUpdatePw();
	};
	...
	this.getConfigJS = function(callback){
		Tools.ajax({
			url: '/staticdata/configjs.js',
			params: {
			},
			success: function(data) {
				callback(data);
			...
			error: function(data) {
				console.log("CONFIGJS_ERROR: " + data);
			}
		});
	};

	this.getGamedatasJS = function(callback){
		Tools.ajax({
			url: '/staticdata/gamedatas.js',
			params: {
			},
			success: function(data) {
				callback(data);
			...
			error: function(data) {
				console.log("GAMEDATASJS_ERROR: " + data);
			}
		});
	};

	this.getParamJS = function(callback){
		Tools.ajax({
			url: '/staticdata/paramcfg.js',
			params: {
			},
			success: function(data) {
				callback(data);
			...
			error: function(data) {
				console.log("PARAMCFGJS_ERROR: " + data);
			}
		});
	};
});
